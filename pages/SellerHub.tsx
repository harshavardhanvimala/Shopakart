    ...
    <div className="min-h-screen bg-[#f1f3f6] dark:bg-obsidian pb-32 animate-in fade-in transition-colors">
      ...
      <main className="px-6 mt-6 space-y-6">
        {lowStockProducts.length > 0 && (
          <div className="bg-rose-50 dark:bg-rose-500/10 border border-rose-100 dark:border-rose-500/20 p-4 rounded-2xl flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <p className="text-xs font-bold text-rose-700 dark:text-rose-400">{lowStockProducts.length} items low in stock</p>
            </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {[
            { id: 'stock', label: 'Inventory', icon: <Package />, action: onAddStock, color: 'text-blue-500', bg: 'bg-blue-50' },
            { id: 'marketing', label: 'AI Marketing', icon: <Sparkles />, action: () => setShowMarketing(true), color: 'text-indigo-500', bg: 'bg-indigo-50' },
            { id: 'deals', label: 'Promotions', icon: <Percent />, action: onViewDeals, color: 'text-pink-500', bg: 'bg-pink-50' },
            { id: 'locations', label: 'Branches', icon: <Map />, action: onViewStores, color: 'text-emerald-600', bg: 'bg-emerald-50' }
          ].map(item => (
            <button 
              key={item.id}
              onClick={item.action}
              className="bg-white dark:bg-white/5 p-5 rounded-3xl border border-slate-100 dark:border-white/10 flex flex-col gap-4 text-left active:scale-95 transition-all group shadow-sm"
            >
              <div className={`w-10 h-10 ${item.bg} dark:bg-white/10 ${item.color} rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform`}>
      ...
      <button 
        onClick={onAddStock}
        className="fixed bottom-32 right-8 w-14 h-14 bg-emerald-600 text-white rounded-[24px] shadow-2xl flex items-center justify-center active:scale-95 transition-all z-40"
      >
    ...
    <div className="fixed inset-0 z-[100] flex items-end justify-center">
       ...
       <div className="relative w-full max-w-md bg-white dark:bg-obsidian rounded-t-[44px] p-8 shadow-2xl animate-in slide-in-from-bottom duration-300">
          ...
          <div className="space-y-4">
             <button 
                onClick={onOpenQR}
                className="w-full p-5 bg-slate-50 dark:bg-white/5 rounded-3xl border border-slate-100 dark:border-white/10 flex items-center gap-4 hover:bg-slate-100 transition-colors"
             >
                <div className="w-12 h-12 bg-white dark:bg-white/10 text-emerald-600 rounded-2xl flex items-center justify-center shadow-sm"><QrCode size={24} /></div>
                <div className="text-left">
                   <p className="font-bold text-sm">Store QR Code</p>
             ...
             <div className="p-6 bg-violet-50 dark:bg-violet-500/10 rounded-[32px] border border-violet-100 dark:border-violet-500/20">
                <div className="flex justify-between items-center mb-4">
                   <span className="text-[10px] font-black text-violet-700 dark:text-violet-400 uppercase tracking-widest">AI Content Studio</span>
                   <button 
