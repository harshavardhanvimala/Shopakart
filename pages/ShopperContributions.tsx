...
const ShopperContributions: React.FC<ShopperContributionsProps> = ({ onBack }) => {
  const stats = [
    { label: 'Reviews', val: '12', icon: <Star size={16} />, color: 'text-amber-500', bg: 'bg-amber-50' },
    { label: 'Impact', val: '450+', icon: <TrendingUp size={16} />, color: 'text-emerald-500', bg: 'bg-emerald-50' },
    { label: 'Badges', val: '3', icon: <Award size={16} />, color: 'text-blue-500', bg: 'bg-blue-50' }
  ];
        ...
        <div className="grid grid-cols-3 gap-3">
          {stats.map((s, i) => (
            <div key={i} className="bg-white dark:bg-white/5 p-4 rounded-[28px] border border-slate-100 dark:border-white/10 shadow-sm text-center">
              <div className={`w-10 h-10 ${s.bg} dark:bg-white/10 ${s.color} rounded-xl flex items-center justify-center mx-auto mb-2`}>
                {s.icon}
              </div>
